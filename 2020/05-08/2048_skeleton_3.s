...
slide:	addi $sp, $sp, -8  # push 
	...
	# $t0: last value of the row or column
	# $t1: 3rd value of the row or column
	# $t2: 2nd value of the row or column
	# $t3: 1st value of the row or column
	# $t4: register to verify if all the elements are zero and counter to avoid an infinite loop
	# $t7: is used as a flag to indicate whether a merge operation is performed.
.data
...
t3ez:	bnez $t3,t3nz	# if $t3 is not 0, go to t3nz (t3 not zero) label
	move $t3,$t2	# move $t2 to $t3
	move $t2,$t1	# move $t1 to $t2
	move $t1,$t0	# move $t0 to $t1
	move $t0,$0	# move 0 to $t0
	j t3ez		# go to t3ez (t3 equal zero) label to verify if $t3 is 0 again

t3nz:	li $t4,1	# to count iterations of this loop
t2ez:	bnez $t2,t2nz	# if $t2 is not 0 go to t2nz (t2 not zero) label
	move $t2,$t1	# move $t1 to $t2
	move $t1,$t0	# move $t0 to $t1
	move $t0,$0	# move 0 to $t0
	addi $t4,$t4,1	# increment the counter in 1
	beq $t4,4,allz	# if $t4==4 it's because there are just zeros from $t2 to $t0 and loop must finish, allz (all zeros)
	j t2ez		# verify if $t2 is 0 again, jump to t2ez (te equal zero)

t2nz:	bnez $t1,t1nz	# if $t1 is not 0, go to t1nz (t1 not zero) label
	move $t1,$t0	# move $t0 to $t1
	move $t0,$0	# move 0 to $t0

t1nz:	bnez $t7,allz	# if there are merges, go to allz label
	bne $t3,$t2,t3net2	# if $t3 is not equal to $t2 go to t3net2 (t3 not equals to t2) label
	add $t3,$t3,$t2	# if $t3 and $t2 are equal, add them
	move $t2,$0	# place 0 on $t2
	li $t7,1	# set the flag to indicate a merge
	j t2net1	# if $t3 is equal to $t2 executes this line, jump to t2net1 (t2 not equal to t1)
t3net2:	bne $t2,$t1,t2net1	# if $t2 is not equal to $t1 go to t2net1 (t2 not equal to t1) label
	add $t2,$t2,$t1	# if $t2 and $t3 are equal, add them
	move $t1,$0	# place 0 on $t1
	li $t7,1	# set the flag to indicate a merge
	j t2et1		# jump to t2et1 (t2 equal to t1) label
...
t2et1:	bnez $t7,t3ez	# if a merge was performed go to t3ez (t3 equal to zero) label
allz:
	#------ Your code ends here ------	
...
#--------------------------------------------------------------------
	...
	#------ Your code starts here ------
	# $t0: counter for the elements of the puzzle_map
	# $a0: address of a puzzle_map
clear_map: li $t0,0
cml:	sw $0,($a0)	# store 0 on the memory direction in $a0
	...
	beq $t0,16,cmend# if t0==16 go to cmend (clear map end) label
	j cml		# jump to cml (clear map label)
...
generate_a_random_tile: la $t0, puzzle_map
	...
	#------ Your code starts here ------
	# $t0: address of a puzzle map
	# $t1: address of the zero_indices array
	# $t2: puzzle_map array index counter
	# $t3: puzzle_map array element
	# $t4: zero elements counter and several uses for the end of this section
	li $t2,0	# puzzle_map array index counter
	li $t4,0	# zero elements counter
grtl:	beq $t2,16,s1end# if index counter is 16, go to s1end (step 1 end) label
	lw $t3,($t0)	# store on $t3 the element on the direction stored on $t0
	bnez $t3,isnz	# if $t3 is not 0 go to isnz (is not zero) label
...
isnz:	addi $t2,$t2,1	# increase index counter
	...
	j grtl		# jump to grtl (generate a random tile label) label to repeat the loop
...
l2or4:	syscall		# stores in $a0 a random integer in [0,5)
	beq $a0,2,s2or4	# if $a0 is 2 go to s2or4 (store 2 or 4) label
	beq $a0,4,s2or4	# if $a0 is 4 go to s2or4 (store 2 or 4) label
	j l2or4		# jump to l2or4 (label 2 or 4) label
s2or4:	sw $a0,X($0)
...
check_win: addi $sp, $sp, -8  # push 
	...
	#------ Your code starts here ------
	# $t0: target value
	# $t1: to increment the memory direction of the puzzle_map elements
	# $t2: value of the puzzle_map element
...
cwl1:	lw $t2,puzzle_map($t1)	# store on $t2 the puzzle_map element pointed
	bge $t2,$t0,cwl2	# if an element on puzzle_map is equal or greater than $t0, go to cwl2 (check win label 2) label
	addi $t1,$t1,4	# increment $t1 to access the next element on puzzle_map
	bne $t1,64,cwl1	# repeat the loop if $t1 is less than 64, if $t1 is >= 64, go to cwl1 (check win label 1) label
	j nowon		# if this executes the player has not reached the target, jumps to nowon (no won) label
...
check_lose: addi $sp, $sp, -8  # push 
	...
	#------ Your code starts here ------
	# $t0: address of puzzle_map
	# $t1: upper limit for find0 loop and one of the elements in a column or row when comparing with his nearest element
	# $t2: value of the puzzle_map element and nearest element in a column or row to compare with value in $t1
	# $t3: upper limit for column or rows when comparing nearest elements
	# $t4: direction in memory of the first element of a row or column to use when comparing nearest elements
	# $t5: maximum limit for columns when comparing nearest elements in a row, and maximum limit for rows when comparing neares elements of columns
	#  0  4  8 12	to understand better the values of $t3 and $t5
	# 16 20 24 28
	# 32 36 40 44
	# 48 52 56 60
	# 64
...
find0:	lw $t2,($t0)		# load in $t2 the puzzle_map element to compare with 0
	beqz $t2,nolose		# if an element is 0 the player has not lose the game, then go to nolose (no lose) label
	addi $t0,$t0,4		# increment to access the next element in puzzle_map
	bne $t0,$t1,find0	# if a 0 is not found on puzzle_map go to find0 (find 0) label, if 0 is not found, executes next line
...
nextr:	lw $t1,($t0)		# nearest elements in a column
	...
	beq $t0,$t3,nextc	# if reached 3rd element in column, go to the nextc (next column) label
	j nextr			# jump to nextr (next row) label
nextc:	addi $t4,$t4,4		# go to the next column
	beq $t4,$t5,nextl	# if $t4==$t5 go to nextl (next label) label
	move $t0,$t4		# moves the direction of the element 1 of next column to $t0
	j nextc2		# jump to nextc2 (next column 2) label, repeat all again for the next column
...
nextc1:	lw $t1,($t0)		# element in a row
	...
	beq $t0,$t3,nextr1	# if t0==t3 go to nextr1 (next row 1) label to increment the index for the next row
	j nextc1		# jump to nextc1 (next column 1) label to repeat again for the next column in the same row
nextr1:	addi $t4,$t4,16		# locate at the beginning of the next row
	beq $t4,$t5,setv0	# if t4==t5 there are not elements that can be merged, so game must finish, then go to setv0 (set $v0) label
	move $t0,$t4		# move the direction of element 1 of next row to $t0
	j nextr2		# jump to nextr2 (next row 2) label repeat all again for the next row
