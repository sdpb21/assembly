...
premier: 		db "Debut du premier programme - addition", 10, 10, 0
inciter: 		db "Entrez deux valeurs entieres : ", 0
somme:			db "La somme est %d", 10, 0
fin1:			db "Fin du premier programme", 10, 10, 0
...
S: 			times 4 db 0 ; 32-bits integer = 4 bytes
...
;		S = N + M;
;		MOV	EAX, 0				; S = 0 ;;
;		ADD	EAX, dword[N]		; S = S + N ;
;		ADD	EAX, dword[M]		; S = S + M ;
;		MOV dword[S], EAX
	
		MOV	EAX, dword[N]		; S = S + N ;
		ADD	EAX, dword[M]		; S = S + M ;
		MOV dword[S], EAX
